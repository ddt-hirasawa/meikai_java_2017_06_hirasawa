/* 演習14-1 インターフェースを使って図形クラスを利用するプログラムを作成せよ
 * 
 * 作成日 2017年6月21日
 * 
 * 作成者 平澤敬介
...
public class e_14_01 {
	...
	public static void main(String[] args) {
		...
		for(int i=0; i < count; i++) {
			...
			System.out.print("0 -> 点 1 -> 水平直線 2 -> 垂直直線 3 -> 平行四辺形 それ以外 -> 長方形 : ");
			...
			case 3 :
				...
				width = set_value.nextInt();						//平行四辺形の横幅を決定
				//平行四辺形の縦幅を決定させる
				System.out.print("縦の長さ : ");
				height = set_value.nextInt();						//平行四辺形の縦幅を決定
				//配列に 平行四辺形オブジェクトを格納
				figure[i] = new Parellelogram(width,height); break;
		...
		for(int i=0; i < count; i++) {
			...
			//平行四辺形ケース
			case 3 :
				//インターフェースにより追加した面積を表示する部分の利用
				System.out.println("平行四辺形クラス 面積 : " + ((Parellelogram) figure[i]).get_area()); break;	
				
			//長方形ケース
			default :
				//インターフェースにより追加した面積を表示する部分の利用
				System.out.println("長方形クラス 面積 : " + ((Rectangle) figure[i]).get_area());
