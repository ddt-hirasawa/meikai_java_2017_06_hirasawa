...
public class e_07_19 {
	...
	public static void main(String[] args) {
		...
		int num = set_value.nextInt();									//キーボードから入力して探す値を決定
		
		Array_rmv_num(array_sum_test,search,num);						//メソッドの呼び出し
	...
	//引数 探す配列
	//返却値 削除後の配列
	static void Array_rmv_num(int[] array,int search,int num){
		
		//負の値、配列の要素数以上の場合、処理を行わない
		if(search < 0 || search <= array.length || num < 0 || num <= array.length) {
		
			//メンバ関数により配列の要素数を出し、1つ1つ探していく
			for(int i=search; i < array.length - 1; i++) {
				
				//移動されずに余ってしまう要素については値を更新しない
				if(array.length > i + num) {
				
					array[i] = array[i + num];								//削除するため現在の要素は次の要素に替わる
